...
```mermaid
flowchart TD
    A[ReferenceSet: one or more named views] --> B[Per-view probe and admission gate]
    B --> C[Pre-Spec Assessment: reconcile views, class, complexity, quality contract]
    B -. when useful .-> A1[Per-view mask, landmark, relative-depth and camera evidence]
    A1 --> C
    ...
    H --> I[Package one matched reference/render sheet per viewId]
    I --> J{Per-view thresholds, then agent vision}
...
### ReferenceSet contract

`ReferenceSet v1` is the intake boundary for both single and multiple images. Each view has a stable
`id`, `role`, and `path`; `primaryViewId` supplies the legacy `sourceImage` mirror. The normalized
manifest also records content hashes so duplicate files cannot masquerade as extra coverage. Schema:
`docs/specs/reference-set.v1.schema.json`.

- `vision-context` is the default. It lets the coding agent reconcile uncalibrated observations,
  but makes no geometric-calibration claim.
- `camera-aware` requires a valid solved-camera block for every view. The primary camera mirrors to
  legacy `referenceCamera`.
- `stage1_intake/process_reference_set.py` probes/admit-checks every view. Assessment and spec
  authoring preserve the full set. `stage4_review/make_multiview_comparison.py` then rejects missing,
  extra, or duplicate render view ids instead of comparing mismatched angles.

The contract improves coverage and review discipline; it is not a photogrammetry, NeRF, or MVS
solver. Programmatic Three.js generation remains the output authority.
...
Each pass has its own acceptance criteria. A pass is marked `continue` only with real renders, matched comparison sheets, agent-vision scores at or above threshold, and every identity-defining feature at or above its own threshold. All explicitly critical source views are gating; if none is marked critical, every supplied view is gating.
...
- **Suitability** — is every admitted view a viable, non-duplicate observation of the same 3D target.
...
- **Screenshot feedback** — `continue` requires a view-matched render/comparison sheet and passing vision score for every gating view, plus banded interior difference: silhouette IoU reads roughly 11% of figure cells and scored a deleted face identically to a finished one.
...
- **Scripts enforce, the model judges.** The Python scripts handle validation, gating, spec authoring, PBR extraction, view-matched comparison packaging, and pipeline state. They never score visuals. The model's tokens go to looking at a bounded set of matched sheets and deciding pass or fail.
...
- **One packaged sheet per view.** Each pass is judged from a bounded `viewReviews[]` bundle; every sheet places a reference beside the render from the same `viewId`.
...
| Script | Role |
| --- | --- |
| `_shared/reference_set.py` | Normalize single/repeated/manifest inputs into `ReferenceSet v1`; enforce primary and duplicate invariants. |
...
| `stage4_review/make_multiview_comparison.py` | Strictly pair source/render by `viewId` and package the matched review bundle. |
| `stage4_review/append_review.py` | Record a per-pass review, including per-view scores and gating evidence. |
...
| `_shared/workflow_state.py`, `state.py`, `next.py` | The resumable ordered checklist, `ReferenceSet` sidecar, and next-action reporter. |
...
- A render plus one matched comparison sheet per reviewed source view, documenting fidelity at each pass.
