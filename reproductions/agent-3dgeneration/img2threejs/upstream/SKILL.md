---
name: img2threejs
description: Turn one or more object/character reference images into a quality-gated, animation-ready procedural Three.js model built in code. Use for image-to-3D reconstruction, detail-accurate object rebuilds, stylized/likeness-maximized human characters, sculpt specs, and staged code generation.
license: Apache-2.0
version: 1.5.1
---
...
Rebuild the object visible in one reference image or a named multi-view `ReferenceSet` as a **code-only** procedural Three.js model,
...
2. **Validate** every source view is a suitable, non-duplicate observation of the same 3D target (`grimoire/intake/validation_rubric.md`).
...
hidden sides; multiple uncalibrated images reduce uncertainty but still do not guarantee exact
geometry. This workflow fuses evidence for agent reasoning and matched-view review; it is not an
MVS/photogrammetry solver.
...
# Multi-view: repeat --reference with stable ids, or pass a ReferenceSet v1 manifest.
python3 forge/state.py init --state .img2threejs/state.json --reference front=<front.png> --reference right=<right.png> --profile <generic|cs2|character> --spec object-sculpt-spec.json
python3 forge/state.py init --state .img2threejs/state.json --reference-set references.json --profile <generic|cs2|character> --spec object-sculpt-spec.json
...
- one or more image paths / screenshots / URLs / attached images (if missing or unreadable, ask); for multiple views use stable ids such as `front`, `right`, `rear`, `detail-handle`
- optional `ReferenceSet v1` manifest (`docs/specs/reference-set.v1.schema.json`): use `vision-context` for uncalibrated evidence and `camera-aware` only when every view has solved camera metadata
...
1. **Analyze every image first** (agent vision, before any script): work the layered observation
   ...
   what each view shows or hides, and reconcile contradictions by `viewId`. Observation before
   inference; controlled 3D vocabulary; 3D object-space not 2D image-space. For a `ReferenceSet`,
   run `forge/stage1_intake/process_reference_set.py references.json --out reference-set-intake.json`;
   it probes and admits every local view. The legacy single-image probe remains
...
2. **Pre-Spec Assessment Gate** — classify + score complexity + write the quality contract:
   `forge/stage2_spec/new_pre_spec_assessment.py "Name" --reference-set references.json --complexity <simple|moderate|complex|ultra-complex> --out assessment.json` (single-image compatibility: use `--image <img>`). Rules: `grimoire/intake/quality_contract.md`.
...
3. Author the spec from the assessment:
   `forge/stage2_spec/new_sculpt_spec.py "Name" --reference-set references.json --assessment assessment.json --manifest cs2-intake.json --out object-sculpt-spec.json` (single-image compatibility: use `--image <img>`).
...
9. Package matched reference/render sheets, then inspect every sheet with agent vision:
   `forge/stage4_review/make_multiview_comparison.py --reference front=<front.png> right=<right.png> --render front=<front-shot.png> right=<right-shot.png> --out-dir comparisons --out matched-review.json`.
   For a single image, `forge/stage4_review/make_comparison_sheet.py --reference <img> --render <shot> --out cmp.png --json` remains valid. Never compare a source view to a render from a different camera.
10. Add `aiVisionScore`, `critical`, and notes to each `matched-review.json.viewReviews[]`, then record the review:
    `forge/stage4_review/append_review.py object-sculpt-spec.json --pass-id <pass> --fidelity <0-1> --action <continue|refine-spec|refine-code|request-input|stop> --summary "..." --view-reviews-json matched-review.json --layer-scores-json '{...}' --feature-reviews-json <f.json> --in-place`.
    `continue` requires every critical view to meet the threshold; if no view is marked critical,
    every view is gating. Legacy single-pair review flags remain supported.
