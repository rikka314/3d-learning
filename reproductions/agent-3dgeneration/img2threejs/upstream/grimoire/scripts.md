...
- `state.py init --state .img2threejs/state.json --reference [VIEW=]IMG [--reference [VIEW=]IMG ...] [--profile generic|cs2|character]`
  creates the local mandatory checklist. Use `--reference-set references.json` instead of repeated
  flags for a `ReferenceSet v1` manifest. It refuses to overwrite existing state and preserves
  `artifacts.reference` as the primary-view compatibility mirror.
...
## stage1_intake/process_reference_set.py
`stage1_intake/process_reference_set.py references.json --out reference-set-intake.json`
normalizes `ReferenceSet v1`, rejects missing/duplicate sources, and runs probe/admission for every
named view. It preserves per-view camera/evidence metadata and emits `proceed` only when no view is
rejected.

## stage2_spec/new_pre_spec_assessment.py
`stage2_spec/new_pre_spec_assessment.py "Name" [--image IMG | --reference-set references.json] [--complexity simple|moderate|complex|ultra-complex] --out assessment.json [--force]`
...
`stage2_spec/new_sculpt_spec.py "Name" [--image IMG | --reference-set references.json] [--assessment assessment.json] --out object-sculpt-spec.json [--force]`
Starter `ObjectSculptSpec` (schema 2.0). With `--assessment` it seeds from the completed gate.
The primary `ReferenceSet` view mirrors to legacy `sourceImage`/`referenceCamera`. Always replace
generic starter `featureReviewTargets` with real identity-defining systems.
...
## stage4_review/make_multiview_comparison.py
`stage4_review/make_multiview_comparison.py --reference front=IMG right=IMG --render front=SHOT right=SHOT --out-dir comparisons --out matched-review.json [--primary-view front]`
rejects missing, extra, or duplicate view ids and emits one matched sheet per view. Inspect each
sheet with agent vision, then add `critical`, `aiVisionScore`, and notes to each
`matched-review.json.viewReviews[]` item before appending the bundle.
...
For multi-view evidence use `--view-reviews-json matched-review.json`; it accepts the comparison
bundle or a bare array. Every critical view must pass; when none is marked critical, all views gate
`action=continue`.
