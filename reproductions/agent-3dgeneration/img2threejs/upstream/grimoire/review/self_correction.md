...
1. Capture or collect a rendered screenshot for every gating source `viewId` (or the sole legacy view).
2. Select at most five critical semantic systems for the current pass and only the suspicious important systems.
3. Create one matched reference/render sheet per source view with `stage4_review/make_multiview_comparison.py`; the legacy single-view route may use `make_comparison_sheet.py`.
4. Inspect each sheet once with your agent's vision and score that view, relevant visual layers, and each selected semantic feature visible in that pair.
...
7. Choose exactly one action:
   ...
   - `request-input`
   - `stop`
8. Record `viewReviews[]`, comparison paths, aggregate score, layer scores, feature scores, and AI critique in `reviewHistory`.

For visual passes, `continue` requires matched render/comparison evidence and an AI vision score at or above threshold for every critical view (all views when none is marked critical), plus every critical feature at or above its own threshold. Without them, the review is not evidence-backed enough. Pixel comparison code is never the acceptance authority.
