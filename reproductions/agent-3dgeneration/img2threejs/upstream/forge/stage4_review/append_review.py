...
from multiview_review import is_number as is_score
from multiview_review import multiview_evidence_failures
...
def visual_acceptance_threshold(spec: dict) -> float:
    ...
    if isinstance(loop, dict):
        ...
        if isinstance(acceptance, dict) and is_score(acceptance.get("threshold")):
            return clamp_score(float(acceptance["threshold"]))
    targets = spec.get("qualityTargets")
    if isinstance(targets, dict) and is_score(targets.get("targetFidelity")):
...
def review_completes_pass(spec: dict, entry: dict, pass_id: str) -> bool:
    ...
    if isinstance(viewpoints, list) and pass_id in VISUAL_PASS_IDS:
        if not {"thickness-axis", "long-axis"}.issubset(set(viewpoints)):
            return False
    view_reviews = entry.get("viewReviews")
    threshold = entry.get("visualAcceptanceThreshold", 0.7)
    has_multiview_evidence = not multiview_evidence_failures(spec, view_reviews, threshold)
    if pass_id in VISUAL_PASS_IDS and not (
        (isinstance(visual, dict) and visual.get("renderScreenshot")) or has_multiview_evidence
    ):
        return False
    if pass_id in VISUAL_PASS_IDS:
        ...
        if float(score) < float(threshold):
            return False
        if not (
            (isinstance(visual, dict) and visual.get("comparisonImage")) or has_multiview_evidence
        ):
            return False
    ...
    if current in VISUAL_PASS_IDS:
        required.extend(
            [
                ...
                "view-matched reference/render comparison sheet(s) for all gating source views",
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--view-reviews-json",
        help="JSON array, comparison-bundle object, or file path containing matched per-view review evidence",
    )
    ...
    view_reviews_payload = load_json_argument(args.view_reviews_json, "--view-reviews-json")
    if isinstance(view_reviews_payload, dict):
        view_reviews = view_reviews_payload.get("viewReviews")
        if not isinstance(view_reviews, list):
            raise ValueError("--view-reviews-json bundle must contain a viewReviews array")
    else:
        view_reviews = view_reviews_payload
    if view_reviews is None:
        view_reviews = []
    if not isinstance(view_reviews, list):
        raise ValueError("--view-reviews-json must be a JSON array")
    ...
    if (
        ...
        and not args.render_screenshot
        and not view_reviews
    ):
        raise ValueError(
    ...
    has_explicit_critical_view = any(
        isinstance(review, dict) and review.get("critical") is True
        for review in view_reviews
    )
    for index, review in enumerate(view_reviews):
        if not isinstance(review, dict):
            raise ValueError(f"view review {index} must be an object")
        view_id = review.get("viewId")
        if not isinstance(view_id, str) or not view_id.strip():
            raise ValueError(f"view review {index}.viewId is required")
        if view_id in seen_view_ids:
            raise ValueError(f"duplicate view review id: {view_id}")
        seen_view_ids.add(view_id)
        score = review.get("aiVisionScore")
        if score is not None and (not is_score(score) or not 0 <= float(score) <= 1):
            raise ValueError(f"view review {view_id}.aiVisionScore must be from 0 to 1")
        if args.action == "continue":
            for field in ("renderScreenshot", "comparisonImage"):
                if not isinstance(review.get(field), str) or not review[field].strip():
                    raise ValueError(f"view review {view_id}.{field} is required for continue")
            if score is None:
                raise ValueError(f"view review {view_id}.aiVisionScore is required for continue")
            is_gating_view = review.get("critical") is True or not has_explicit_critical_view
            if is_gating_view and float(score) < threshold:
                label = "critical view" if review.get("critical") is True else "view"
                raise ValueError(
                    f"{label} {view_id} score {float(score):.3f} is below threshold {threshold:.3f}"
                )
    if args.action == "continue" and view_reviews:
        evidence_failures = multiview_evidence_failures(spec, view_reviews, threshold)
        if evidence_failures:
            raise ValueError(evidence_failures[0])
    if args.require_screenshot_files:
        for review in view_reviews:
            view_id = review["viewId"]
            validate_optional_file(review.get("referenceScreenshot"), f"view review {view_id}.referenceScreenshot")
            validate_optional_file(review.get("renderScreenshot"), f"view review {view_id}.renderScreenshot")
            validate_optional_file(review.get("comparisonImage"), f"view review {view_id}.comparisonImage")
    layer_scores = None
    if args.layer_scores_json:
        ...
        for key, value in layer_scores.items():
            if not isinstance(key, str) or not is_score(value):
    ...
    if args.pass_id in VISUAL_PASS_IDS and args.action == "continue":
        if not args.comparison_image and not view_reviews:
            raise ValueError(
                "visual pass cannot use action=continue without --comparison-image; "
                "create one with stage4_review/make_comparison_sheet.py"
            )
        if args.ai_vision_score is None and not view_reviews:
            raise ValueError(
                "visual pass cannot use action=continue without --ai-vision-score; "
                "AI vision must review the comparison sheet(s)"
            )
        if args.ai_vision_score is not None and clamp_score(args.ai_vision_score) < threshold:
            raise ValueError(
    ...
    multiview_scores = [
        clamp_score(review["aiVisionScore"])
        for review in view_reviews
        if is_score(review.get("aiVisionScore"))
        and (review.get("critical") is True or not has_explicit_critical_view)
    ]
    entry = {
        ...
        "aiVisionScore": (
            clamp_score(args.ai_vision_score)
            if args.ai_vision_score is not None
            else (min(multiview_scores) if multiview_scores else None)
        ),
    ...
    if view_reviews:
        entry["viewReviews"] = view_reviews
