...
def make_spec(target_name: str, image: str | None, assessment_payload: dict | None = None) -> dict:
    ...
    if assessment_payload:
        ...
        incoming_reference_set = assessment_payload.get("referenceSet")
        ...
        if isinstance(incoming_reference_set, dict):
            reference_set = incoming_reference_set
    primary_view = None
    ...
    reference_camera = (
        primary_view.get("camera")
        if isinstance(primary_view, dict) and isinstance(primary_view.get("camera"), dict)
        else default_reference_camera
    )
    spec = {
        ...
        "selfCorrectLoop": {
            ...
            "screenshotPolicy": {
                ...
                "minimumEvidence": "Each visual pass needs view-matched reference/render screenshot pairs, one comparison sheet and AI vision score per gating source view, layer scores, and critique before choosing continue.",
        ...
        "sculptPipeline": {
            ...
            "nextRequiredEvidence": [
                ...
                "view-matched reference/render comparison sheet(s)",
    ...
    if reference_set is not None:
        spec["referenceSet"] = reference_set
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--reference-set",
        type=Path,
        help="ReferenceSet v1 JSON manifest. Its primary view mirrors to legacy fields.",
    )
    ...
    if args.reference_set:
        ...
        except (OSError, json.JSONDecodeError, ReferenceSetError) as error:
            parser.error(f"invalid reference-set manifest: {error}")
    if reference_set is not None:
        assessment = dict(assessment) if isinstance(assessment, dict) else {}
        assessment["referenceSet"] = reference_set
