...
from forge._shared.reference_set import (  # noqa: E402
    ReferenceSetError,
...
class PreSpecPayload(PreSpecPayloadRequired, total=False):
    ...
    referenceSet: dict[str, JsonValue]


def _primary_reference_view(reference_set: dict) -> dict | None:
...
def make_payload(
    ...
    primary_view = _primary_reference_view(reference_set) if reference_set is not None else None
    source_image = (
    ...
    if reference_set is not None:
        payload["referenceSet"] = reference_set
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--reference-set",
        type=Path,
        help="ReferenceSet v1 JSON manifest. Its primary view mirrors to sourceImage.",
    )
    parser.add_argument(
        "--complexity",
