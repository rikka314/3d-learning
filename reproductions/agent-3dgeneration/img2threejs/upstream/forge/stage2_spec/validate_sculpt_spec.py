...
from reference_set import ReferenceSetError, normalize_reference_set
...
def _valid_reference_camera(camera: Any) -> bool:
    ...
    return (
        camera.get("solved") is True
        and is_number(fov)
        and 0.0 < float(fov) < 180.0
        and is_number(aspect)
        and float(aspect) > 0.0
        and isinstance(orientation, dict)
        and all(is_number(orientation.get(axis)) for axis in ("yaw", "pitch", "roll"))
        and isinstance(position, list)
        and len(position) == 3
        and all(is_number(value) for value in position)
    )


def validate_reference_set(spec: dict[str, Any], errors: list[str]) -> None:
    reference_set = spec.get("referenceSet")
    if reference_set is None:
        return
    if not isinstance(reference_set, dict):
        errors.append("referenceSet must be an object")
        return
    try:
        normalize_reference_set(manifest=reference_set, require_files=False)
    except ReferenceSetError as error:
        errors.append(f"referenceSet is invalid: {error}")
        return

    views = reference_set.get("views")
    primary_id = reference_set.get("primaryViewId")
    if not isinstance(views, list):
        errors.append("referenceSet.views must be an array")
        return
    ids = [view.get("id") for view in views if isinstance(view, dict)]
    duplicate_ids = sorted({view_id for view_id in ids if ids.count(view_id) > 1})
    if duplicate_ids:
        errors.append(f"referenceSet has duplicate view ids: {duplicate_ids}")
    primary_view = next(
        (
            view
            for view in views
    ...
    if primary_view is None:
        errors.append("referenceSet primaryViewId must identify one of its views")
        return
    primary_path = primary_view.get("path")
    if spec.get("sourceImage") != primary_path:
        errors.append("sourceImage must mirror the referenceSet primary view path")

    mode = reference_set.get("mode", "vision-context")
    if mode == "camera-aware":
        for index, view in enumerate(views):
            if not isinstance(view, dict) or not _valid_reference_camera(view.get("camera")):
                errors.append(f"referenceSet camera-aware view[{index}] has invalid camera metadata")
        primary_camera = primary_view.get("camera")
        if spec.get("referenceCamera") != primary_camera:
            errors.append("referenceCamera must mirror the referenceSet primary view camera")
VALID_PRIMITIVES = {
    "box",
    "sphere",
    "ellipsoid",
...
def validate_view_reviews(entry: dict[str, Any], label: str, errors: list[str]) -> None:
    reviews = entry.get("viewReviews")
    if reviews is None:
        return
    if not isinstance(reviews, list):
        errors.append(f"{label}.viewReviews must be an array")
        return
    seen: set[str] = set()
    for index, review in enumerate(reviews):
        item_label = f"{label}.viewReviews[{index}]"
        if not isinstance(review, dict):
            errors.append(f"{item_label} must be an object")
            continue
        view_id = review.get("viewId")
        if not isinstance(view_id, str) or not view_id.strip():
            errors.append(f"{item_label}.viewId is required")
        elif view_id in seen:
            errors.append(f"{label}.viewReviews has duplicate viewId {view_id!r}")
        else:
            seen.add(view_id)
        for field in ("referenceScreenshot", "renderScreenshot", "comparisonImage", "notes"):
            value = review.get(field)
            if value is not None and not isinstance(value, str):
                errors.append(f"{item_label}.{field} must be a string")
        score = review.get("aiVisionScore")
        if score is not None:
            validate_unit_interval(score, f"{item_label}.aiVisionScore", errors)
...
def validate_review_history(spec: dict[str, Any], errors: list[str], warnings: list[str]) -> None:
    ...
    for index, entry in enumerate(history):
        ...
        validate_view_reviews(entry, f"reviewHistory[{index}]", errors)
        pass_id = entry.get("passId")
        score = entry.get("aiVisionScore")
        threshold = entry.get("visualAcceptanceThreshold", 0.7)
        if score is not None:
            validate_unit_interval(score, f"reviewHistory[{index}].aiVisionScore", errors)
        if "visualAcceptanceThreshold" in entry:
            validate_unit_interval(
                threshold,
                f"reviewHistory[{index}].visualAcceptanceThreshold",
                errors,
            )
        view_reviews = entry.get("viewReviews")
        multiview_failures = multiview_evidence_failures(spec, view_reviews, threshold)
        has_multiview_evidence = (
            isinstance(view_reviews, list)
            and bool(view_reviews)
            and not multiview_failures
        )
        if (
            pass_id in VISUAL_PASS_IDS
            and action == "continue"
            and not (isinstance(visual, dict) and visual.get("renderScreenshot"))
            and not has_multiview_evidence
        ):
            warnings.append(
        ...
        if pass_id in VISUAL_PASS_IDS and action == "continue":
            if (
                (not isinstance(visual, dict) or not visual.get("comparisonImage"))
                and not has_multiview_evidence
            ):
                warnings.append(
            ...
            if isinstance(view_reviews, list) and view_reviews:
                for failure in multiview_failures:
                    warnings.append(
                        f"quality: reviewHistory[{index}] multiview evidence invalid: {failure}"
                    )
            loop = spec.get("selfCorrectLoop")
...
def review_completes_pass(
    ...
    if pass_id in VISUAL_PASS_IDS:
        ...
        if (
            ...
            or not is_number(threshold)
            or not 0.0 <= float(threshold) <= 1.0
            or float(score) < float(threshold)
        ):
            return False
        has_multiview_evidence = not multiview_evidence_failures(
            spec,
            entry.get("viewReviews"),
            threshold,
        )
        if not has_single_view_evidence and not has_multiview_evidence:
            return False
...
def validate_spec(spec: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    validate_reference_set(spec, errors)
