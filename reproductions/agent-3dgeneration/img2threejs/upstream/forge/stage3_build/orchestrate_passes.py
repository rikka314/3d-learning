...
from multiview_review import is_number as is_review_number
...
def review_completes_pass(spec: dict[str, Any], entry: dict[str, Any], pass_id: str) -> bool:
    ...
    if pass_id in VISUAL_PASS_IDS:
        ...
        if (
            not is_review_number(score)
            or not 0.0 <= float(score) <= 1.0
            or not is_review_number(threshold)
            or not 0.0 <= float(threshold) <= 1.0
            or float(score) < float(threshold)
        ):
            return False
        has_single_view_evidence = bool(
            visual.get("renderScreenshot") and visual.get("comparisonImage")
        )
        has_multiview_evidence = not multiview_evidence_failures(
            spec, entry.get("viewReviews"), threshold
        )
...
def next_required_evidence(spec: dict[str, Any], pass_id: str) -> list[str]:
    ...
    if pass_id in VISUAL_PASS_IDS:
        ...
        evidence.append("view-matched reference/render comparison sheet(s) for AI vision review")
...
def pass_specific_evidence(pass_id: str) -> list[str]:
    ...
    if pass_id == "material-pass":
        return [
            ...
            "AI vision score for every gating view-matched comparison sheet meeting the visual acceptance threshold",
        ]
