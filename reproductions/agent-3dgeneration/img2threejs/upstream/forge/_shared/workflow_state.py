...
from reference_set import ReferenceSetError, normalize_reference_set, primary_view
...
SETUP_STEPS: Final = (
    ("image-analysis", "Read grimoire/intake/image_analysis.md and analyze every view in {reference_set}"),
    (
        "reference-suitability",
        "Read grimoire/intake/validation_rubric.md and record per-view plus reference-set suitability for {reference_set}",
    ),
    (
        "reference-admission",
        "python3 forge/stage1_intake/process_reference_set.py {reference_set} --out reference-set-intake.json",
    ),
    ("local-spec-search", "Run the local evidence search before authoring the assessment"),
    ("pre-spec-assessment", "python3 forge/stage2_spec/new_pre_spec_assessment.py \"<name>\" --reference-set {reference_set} --out assessment.json"),
    ...
    ("spec-authoring", "python3 forge/stage2_spec/new_sculpt_spec.py \"<name>\" --reference-set {reference_set} --assessment assessment.json --out object-sculpt-spec.json"),
    (
        "material-evidence",
...
PASS_STEPS: Final = (
    ...
    ("ai-review-recorded", "Create view-matched comparison sheet(s), inspect them with agent vision, and append exactly one review action"),
...
def new_state(
    reference: str | list[str] | dict[str, Any],
    *,
    profile: str = "generic",
    spec: str = "",
    max_per_pass: int = 3,
    ...
    legacy_reference = reference if isinstance(reference, str) and "=" not in reference else None
    if isinstance(reference, dict):
        reference_set = normalize_reference_set(manifest=reference, require_files=False)
    else:
        reference_set = normalize_reference_set(references=reference, require_files=False)
    primary_reference = legacy_reference or primary_view(reference_set)["path"]
    state = {
...
def validate_state(state: Any) -> dict[str, Any]:
    ...
    reference_set = artifacts.get("referenceSet")
    if reference_set is not None:
        try:
            normalize_reference_set(manifest=reference_set, require_files=False)
        except ReferenceSetError as error:
            raise WorkflowStateError(f"state artifacts.referenceSet is invalid: {error}") from error
...
def _format_command(state: dict[str, Any], entry: dict[str, Any]) -> str:
    ...
    return command.format(
        ...
        reference_set=shlex.quote(
            str(artifacts.get("referenceSetManifest") or artifacts.get("reference") or "<reference-set>")
        ),
