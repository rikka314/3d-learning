...
You give it one reference image or a named multi-view `ReferenceSet`. It produces a `THREE.Group` factory written in TypeScript that recreates that object from primitives, procedural shaders, and generated geometry — with a runtime hierarchy (pivots, sockets, colliders) so the result is ready to animate, not an inert lump.
...
- **Multi-view workflow context.** `ReferenceSet v1` gives every source a stable `viewId`, runs intake per view, carries the complete set through assessment/spec authoring, and reviews each render only against its matching source view. `vision-context` accepts uncalibrated evidence; `camera-aware` requires solved camera metadata for every view. This is evidence fusion for the coding agent, not automatic MVS or photogrammetry.
...
A staged sculpting pipeline turns the reference set into one shared spec, then generates and vision-reviews one build pass at a time — `blockout → structural → form → material → surface → lighting → interaction → optimization` — self-correcting until every identity-defining feature and gating source view clears its threshold.
...
A staged sculpting pipeline turns the reference set into a spec, then generates and vision-reviews one build pass at a time — `blockout → structural → form → material → surface → lighting → interaction → optimization` — self-correcting until every identity-defining feature clears its threshold. Deterministic Python scripts handle validation, view pairing, and gating; model tokens are spent only on visual judgment and code.
...
3. **Follow the pipeline** — the skill validates the image, writes an assessment and spec, generates the factory pass by pass, and shows you a side-by-side comparison at each step until the render matches.
   ...
   For multiple views, repeat `--reference` with stable ids or provide a manifest conforming to
   `docs/specs/reference-set.v1.schema.json`:

   ```bash
   python3 forge/state.py init --reference front=<front.png> --reference right=<right.png> --profile character --spec object-sculpt-spec.json
   # equivalent: python3 forge/state.py init --reference-set references.json --profile character --spec object-sculpt-spec.json
   python3 forge/next.py --state .img2threejs/state.json
   ```
...
Multi-view uses the same build generator and substitutes a normalized manifest at intake/spec time:

```bash
python3 forge/stage1_intake/process_reference_set.py references.json --out reference-set-intake.json
python3 forge/stage2_spec/new_pre_spec_assessment.py "Name" --reference-set references.json --out assessment.json
python3 forge/stage2_spec/new_sculpt_spec.py "Name" --reference-set references.json --assessment assessment.json --out spec.json
python3 forge/stage4_review/make_multiview_comparison.py --reference front=<front.png> right=<right.png> --render front=<front-shot.png> right=<right-shot.png> --out-dir comparisons --out matched-review.json
```
...
- **Scripts enforce, the model judges.** The Python scripts handle validation, gating, spec authoring, PBR extraction, matched comparison-sheet packaging, and pipeline state. They never score visuals. The model's tokens go to looking at bounded, view-matched sheets and deciding pass or fail.
...
- **One packaged sheet per view.** Each pass is judged from a bounded `viewReviews[]` bundle in which every reference is beside the render from the same `viewId`, not a scattering of unmatched screenshots.
...
| Script | Role |
| --- | --- |
| `_shared/reference_set.py` | Normalize single/multi-image inputs into `ReferenceSet v1`, enforce primary-view and duplicate invariants. |
| `stage1_intake/probe_image.py` | Image metadata and obvious technical issues (not a visual check). |
| `stage1_intake/process_reference_set.py` | Probe and run admission for every named source view. |
...
| `stage4_review/make_multiview_comparison.py` | Pair source and render strictly by `viewId`, emitting one sheet per view. |
| `stage4_review/append_review.py` | Record a per-pass review, including gating `viewReviews[]` evidence. |
...
- **v2.0 — The Procedural World Update**: camera-optimized multi-view geometric reconstruction beyond the current workflow-level evidence fusion, procedural city generation, semantic world understanding, plugin ecosystem and API.
...
A single image cannot reveal hidden sides or guarantee exact geometry. Multiple images help only to the extent that they show new, consistent evidence; uncalibrated `vision-context` views do not turn this code-generation workflow into photogrammetry, NeRF, or MVS. The skill states plainly when output is approximate, stylized, or low-poly, and reports unseen regions rather than faking confidence. It is strong for hard-surface objects; characters are stylized reconstructions, not photoreal likeness. "This cannot reach the requested fidelity from these references" is a valid, expected result.
