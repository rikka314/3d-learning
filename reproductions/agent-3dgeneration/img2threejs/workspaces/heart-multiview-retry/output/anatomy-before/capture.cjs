...
(async () => {
  ...
  const sources = ['src/createHeartModel.ts', 'src/heartMaterials.ts', 'src/main.ts', 'src/style.css', 'index.html', 'anatomy-layout.json', 'object-sculpt-spec.json', 'references.json'];
  ...
  try {
    ...
    const servedSources = [];
    for (const file of ['src/createHeartModel.ts', 'src/heartMaterials.ts', 'src/main.ts', 'src/style.css', 'anatomy-layout.json']) {
    ...
    const runtime = await page.evaluate(() => {
      ...
      root.traverse(object => {
        ...
        if (object.name === 'ventricular-body-surface') {
          ...
          meshes.push({ id: object.name, vertices, normals, indices });
        }
      });
      return { invalidVertices, geometry: { meshes }, performance: window.__RENDER_INFO__(), parts: window.__PART_MANIFEST__() };
    });
    ...
    const result = { stage, errors, captures, snapshot, servedSources, textureHashes, ...runtime };
