...
const referenceVerticalExtent = 5.0;
...
renderer.toneMappingExposure = captureMode ? 0.92 : 0.96;
...
scene.environmentIntensity = captureMode ? 0.48 : 0.56;
...
const lightMode = requestedLight === 'reference' || requestedLight === 'grazing' ? requestedLight : 'neutral';
const lightSettings = {
  reference: {
    hemi: [0xfff5ee, 0x655b5a, 0.62] as const,
    key: [0xffe7dc, 2.3, [-4.2, 5.4, 6.5]] as const,
    fill: [0xdde8ff, 0.56, [4.5, 1.2, 4.2]] as const,
    rim: [0xffc9ba, 0.54, [3.6, 3.2, -5.5]] as const,
  },
  neutral: {
    hemi: [0xfff9f2, 0x5d5858, 0.55] as const,
    key: [0xffe9df, 2.16, [-4.2, 5.2, 6.6]] as const,
    fill: [0xdbe7ff, 0.62, [4.6, 1.5, 4.4]] as const,
    rim: [0xffc7b7, 0.48, [3.8, 3.4, -5.8]] as const,
  },
  grazing: {
    hemi: [0xffeee7, 0x3d3335, 0.38] as const,
    key: [0xffd8c8, 3.15, [-6, 1.6, 2.6]] as const,
    fill: [0xcbdcff, 0.46, [4.8, 0.8, 3.2]] as const,
    rim: [0xffb8a3, 0.72, [3.4, 4.2, -5.4]] as const,
  },
} as const;
...
const rim = new THREE.DirectionalLight(installedLights.rim[0], installedLights.rim[1]);
rim.name = `${lightMode}-rim-${installedLights.rim[1]}`;
rim.position.set(installedLights.rim[2][0], installedLights.rim[2][1], installedLights.rim[2][2]);

key.castShadow = true;
key.shadow.mapSize.set(2048, 2048);
key.shadow.camera.near = 0.5;
key.shadow.camera.far = 18;
key.shadow.camera.left = -3.25;
key.shadow.camera.right = 3.25;
key.shadow.camera.top = 3.25;
key.shadow.camera.bottom = -3.25;
...
const cameraFill = new THREE.DirectionalLight(0xfff1e8, 0.2);
cameraFill.name = 'camera-follow-fill';
cameraFill.position.set(0, 0, 1);
...
// One shared framing center: the posterior great vessels bias the overall bounds.
// Preserve true front/side directions while centering both lateral projections.
const viewCenter = new THREE.Vector3(0, 0, -0.20);
...
function setView(name: string): void {
  ...
  controls.update();
  camera.position.copy(position).add(viewCenter);
  camera.up.set(0, 1, 0);
