...
当前已完成表面优化：用连续程序化细节替换重复图片纹理，柔化反射与阴影，提高管壁及心房/心耳的曲面细分。解剖布局与冠脉源码保持一致；最终版本重新通过36项结构、108组血管间距和12项交互检查。对比见 [`output/material-comparison.html`](output/material-comparison.html)，详细记录见 [`surface-review.md`](surface-review.md)，最终证据为 `output/material-final/`。
...
- `verify-vessel-clearance.cjs`：13条大血管的72个非连接管对、9条冠状动脉与4条心静脉的36个动静脉管对。采用501点轴线采样、实际顶点环半径和采样余量；属于有范围的几何检查，不是精确全模型无自交证明。
