...
export function createHeartMaterials(): {
  ...
  const ready = Promise.all(Object.entries(repeats).map(async ([name, repeat]) => {
    ...
    // The epicardial surface is continuous; this relief is visual texture, not exposed muscle cells.
    material.normalScale.setScalar(name === 'fat' ? 0.15 : name === 'myocardium' ? 0.075 : 0.12);
