...
export function createHeartModel(): THREE.Group {
  ...
  for (const part of layout.atria) {
    ellipsoid(part.id, part.label, new THREE.Vector3().fromArray(part.center),
      new THREE.Vector3().fromArray(part.scale), part.rotation);
  }
  ...
  for (const vessel of layout.greatVessels) {
    ...
    if (vessel.id === 'pulmonary-trunk' || vessel.id === 'aortic-arch') {
      points.unshift(points[0].clone().add(new THREE.Vector3(0, -0.27, -0.14)));
      radii.unshift(0.16);
    }
