...
(async () => {
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      ...
      // A fixed, independent inventory: adding visual density must not introduce
      // disconnected colored strokes or silently exempt vessels from validation.
      const surfaceDetails = [
        ...[1,2,3].map(n => [`lad-surface-branch-${n}`, 'left-anterior-descending-artery', 'artery']),
        ...[1,2].map(n => [`right-marginal-surface-branch-${n}`, 'right-marginal-branch', 'artery']),
        ...[1,2].map(n => [`pda-surface-branch-${n}`, 'posterior-descending-artery', 'artery']),
        ...[1,2].map(n => [`obtuse-marginal-surface-branch-${n}`, 'obtuse-marginal-branch', 'artery']),
        ...[1,2].map(n => [`great-cardiac-surface-tributary-${n}`, 'great-cardiac-vein', 'vein']),
        ...[1,2].map(n => [`middle-cardiac-surface-tributary-${n}`, 'middle-cardiac-vein', 'vein']),
      ];
      for (const [id, parent, kind] of surfaceDetails) {
        const metadata = nodes[id]?.userData.anatomy;
        const present = paths[id] && paths[parent];
        const junction = present ? (kind === 'vein' ? last(id) : first(id)) : null;
        const gap = junction ? distanceTo(junction, parent) : null;
        add(`${id}-connection`, present && gap < .018 && metadata?.parentVessel === parent
          && metadata?.kind === kind && metadata?.illustrative === true,
        { expectedParent: parent, declaredParent: metadata?.parentVessel, centerlineGap: gap, illustrative: metadata?.illustrative });
        if (!present) {
          add(`${id}-surface-course`, false, 'Missing required illustrative vessel');
          add(`${id}-distal-taper`, false, 'Missing required illustrative vessel');
          add(`${id}-surface-region-label`, false, 'Missing required illustrative vessel');
          continue;
        }
        const distal = kind === 'vein' ? first(id) : last(id);
        let buried = 0, samples = 0;
        for (let i = 8; i < paths[id].length - 8; i += 12) {
          if (inMesh(v(paths[id][i]), envelopeMesh)) buried++;
          samples++;
        }
        add(`${id}-surface-course`, buried === 0 && distal.y < junction.y - .10,
          { buriedCenterlineSamples: buried, samples, distal: distal.toArray(), junction: junction.toArray(),
            scope: 'Exterior schematic course only; no patient-specific territory or branch-count claim' });
        const proximalRadius = ringRadius(id, kind === 'vein');
        const distalRadius = ringRadius(id, kind !== 'vein');
        add(`${id}-distal-taper`, proximalRadius > 0 && distalRadius > 0 && distalRadius < proximalRadius * .5,
          { proximalRadius, distalRadius });
        // Read which semantic ventricular surface lies under the actual distal
        // branch, independently of its text label or its declared parent vessel.
        const origin = root.localToWorld(distal.clone());
        const inward = root.localToWorld(new THREE.Vector3(0, distal.y, 0)).sub(origin).normalize();
        const ray = new THREE.Raycaster(origin.clone().addScaledVector(inward, -.01), inward);
        const hit = ray.intersectObjects([meshes['left-ventricle-surface'], meshes['right-ventricle-surface']], false)[0];
        const chamberLabel = hit?.object.name === 'left-ventricle-surface' ? '左心室'
          : hit?.object.name === 'right-ventricle-surface' ? '右心室' : null;
        add(`${id}-surface-region-label`, chamberLabel && metadata.region.startsWith(chamberLabel),
          { renderedSurface: hit?.object.name, regionLabel: metadata.region,
            scope: 'Consistency with the modeled surface below the distal branch, not a physiologic perfusion or drainage territory validation' });
      }
