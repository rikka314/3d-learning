...
const lightSettings = {
  reference: {
    hemi: [0xfff3ea, 0x573638, 2] as const,
    key: [0xffe8dc, 3, [-3, 5, 6]] as const,
    fill: [0xdde8ff, 1.4, [4, 1.5, 4]] as const,
  },
  neutral: {
...
canvas.addEventListener('pointerup', (event) => {
  ...
  pointerStart = null;
  if (event.button !== 0 || !start || start.id !== event.pointerId || Math.hypot(event.clientX - start.x, event.clientY - start.y) > 5) return;
