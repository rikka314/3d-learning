...
export function createHeartModel(): THREE.Group {
  ...
  for (const item of fatPaths) {
    ...
    const bed = tubeGeometry(curve, [item.radius, item.radius * 0.96, item.radius * 0.5], 130, 18);
    const bedPosition = bed.getAttribute('position');
    for (let row = 0; row <= 130; row++) {
      const center = curve.getPointAt(row / 130);
      for (let side = 0; side <= 18; side++) {
        const vertex = row * 19 + side;
