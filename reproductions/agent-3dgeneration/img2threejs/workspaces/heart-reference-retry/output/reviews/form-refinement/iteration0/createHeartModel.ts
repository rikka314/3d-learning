...
const BUILD_PASS = 'form-refinement';
...
export function createHeartModel(): THREE.Group {
  ...
  function ellipsoid(id: string, label: string, center: THREE.Vector3, scale: THREE.Vector3, rotation: number): THREE.Group {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      const amplitude = auricle ? 0.072 : 0.023;
  ...
  for (const vessel of layout.greatVessels) {
    ...
    if (vessel.id === 'pulmonary-trunk') { points.unshift(pixel(516, 590, 0.22)); radii.unshift(0.14); }
    ...
    mesh(`${vessel.id}-wall`, node, tubeGeometry(curve, radii, 110, 40), material);
    if (vessel.open) {
  ...
  for (const item of coronaryPaths) {
    ...
    const path = new SurfaceCurve(item.path, item.front ? 0.093 : 0.008, item.front);
    ...
    const vein = new SurfaceCurve(shifted, item.front ? 0.087 : 0.007, item.front);
    ...
    for (let i = 0; i < count; i++) {
      ...
      branch.getPoint = (u, target) => {
        ...
        p.z += (item.front ? 1 : -1) * (item.front ? 0.081 : 0) * Math.exp(-u * 22);
        return p;
      };
      const radius = artery ? 0.0105 : 0.008;
      branches[artery ? 0 : 1].push(tubeGeometry(branch, [radius, radius * 0.80, 0.002], 40, 8));
      if (i % 2 === 0) {
        ...
        branches[artery ? 0 : 1].push(tubeGeometry(twig, [radius * 0.5, 0.001], 25, 7));
      }
    }
  ...
  for (const item of fatPaths) {
    ...
    const lobules = new THREE.InstancedMesh(new THREE.SphereGeometry(1, 12, 9), materials.fat, count);
    ...
    for (let i = 0; i < count; i++) {
      ...
      transform.position.set(x, y, surfaceDepth(x, y) + 0.027);
      transform.scale.set(0.025 + rng() * 0.022, 0.040 + rng() * 0.028, 0.023 + rng() * 0.022);
      transform.rotation.set((rng() - 0.5) * 0.5, (rng() - 0.5) * 0.5, (rng() - 0.5) * 1.5);
      transform.updateMatrix();
      lobules.setMatrixAt(i, transform.matrix);
      lobules.setColorAt(i, new THREE.Color().setHSL(0.07 + rng() * 0.025, 0.38 + rng() * 0.16, 0.68 + rng() * 0.18));
    }
